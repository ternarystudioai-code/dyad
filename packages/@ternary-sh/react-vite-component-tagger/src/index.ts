 ...
 */
export default function ternaryTagger(): Plugin {
  return {
    name: "vite-plugin-ternary-tagger",
    apply: "serve",
    enforce: "pre",
    ...
    async transform(code: string, id: string) {
      try {
        ...
        walk(ast as any, {
          enter(node: any) {
            try {
              ...
              // ── 2. Check whether the tag already has data-ternary-id ───────────────
              ...
              console.warn(
                `[ternary-tagger] Warning: Failed to process JSX node in ${id}:`,
                error,
              );
            }
          },
        ...
        console.warn(
          `[ternary-tagger] Warning: Failed to transform ${id}:`,
          error,
        );
        return null;
      }
