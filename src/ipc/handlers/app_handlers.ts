...
function listenToProcess({
  ...
  spawnedProcess.stdout?.on("data", async (data) => {
    ...
    // isNeon because: 1) only Neon apps (for the official Ternary templates) should
...
async function executeAppInDocker({
  ...
  const containerName = `ternary-app-${appId}`;
  ...
  const dockerfilePath = path.join(appPath, "Dockerfile.ternary");
...
`;
  ...
  const buildProcess = spawn(
    "docker",
    ["build", "-f", "Dockerfile.ternary", "-t", `ternary-app-${appId}`, "."],
    {
      cwd: appPath,
      stdio: "pipe",
    },
  ...
  const process = spawn(
    ...
    [
      ...
      `${appPath}:/app`,
      "-v",
      `ternary-pnpm-${appId}:/app/.pnpm-store`,
      "-e",
      ...
      return withLock(appId, async () => {
        try {
          ...
          if (removeNodeModules) {
            ...
            if (runtimeMode === "docker") {
              logger.log(
                `Docker mode detected for app ${appId}. Removing Docker volumes ternary-pnpm-${appId}...`,
              );
              try {
                await removeDockerVolumesForApp(appId);
                logger.log(
                  `Removed Docker volumes for app ${appId} (ternary-pnpm-${appId}).`,
                );
              } catch (e) {
