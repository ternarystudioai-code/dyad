...
import {
  ...
  getTernaryChatSummaryTag,
  getTernaryCommandTags,
...
async function getCodebaseTokenCount(
  ...
  const codebase = (
    await extractCodebase({
      appPath: getTernaryAppPath(appPath),
  ...
  return withLock("get-proposal:" + chatId, async () => {
    ...
    try {
      ...
      if (
        ...
        const proposalTitle = getTernaryChatSummaryTag(messageContent);

        const proposalWriteFiles = getTernaryWriteTags(messageContent);
        const proposalRenameFiles = getTernaryRenameTags(messageContent);
        const proposalDeleteFiles = getTernaryDeleteTags(messageContent);
        const proposalExecuteSqlQueries =
          getTernaryExecuteSqlTags(messageContent);
        const packagesAdded = getTernaryAddDependencyTags(messageContent);
      ...
      if (latestAssistantMessage?.content) {
        const writeTags = getTernaryWriteTags(latestAssistantMessage.content);
        ...
        const commandTags = getTernaryCommandTags(
          latestAssistantMessage.content,
        );
...
const approveProposalHandler = async (
  ...
  const chatSummary = getTernaryChatSummaryTag(messageToApprove.content);
