(() => {
  const OVERLAY_ID = "__ternary_overlay__";
  let overlay, label;
  ...
  function updateOverlay(el, isSelected = false) {
    ...
    const name = el.dataset.ternaryName || "<unknown>";
    const file = (el.dataset.ternaryId || "").split(":")[0];
  ...
  function onMouseMove(e) {
    ...
    let el = e.target;
    while (el && !el.dataset.ternaryId) el = el.parentElement;
  ...
  function onClick(e) {
    ...
    window.parent.postMessage(
      {
        type: "ternary-component-selected",
        id: state.element.dataset.ternaryId,
        name: state.element.dataset.ternaryName,
      },
      "*",
    );
  }
  ...
  window.addEventListener("message", (e) => {
    ...
    if (e.data.type === "activate-ternary-component-selector") activate();
    if (e.data.type === "deactivate-ternary-component-selector") deactivate();
  });

  function initializeComponentSelector() {
    if (!document.body) {
      console.error(
        "Ternary component selector initialization failed: document.body not found.",
      );
      return;
    }
    setTimeout(() => {
      if (document.body.querySelector("[data-ternary-id]")) {
        window.parent.postMessage(
          {
            type: "ternary-component-selector-initialized",
          },
          "*",
        );
        console.debug("Ternary component selector initialized");
      } else {
        console.warn(
          "Ternary component selector not initialized because no DOM elements were tagged",
        );
      }
    }, 0);
  }
